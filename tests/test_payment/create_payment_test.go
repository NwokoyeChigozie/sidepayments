...
func TestCreatePayment(t *testing.T) {
	...
	trans := payment.Controller{Db: db, Validator: validatorRef, Logger: logger, ExtReq: request.ExternalRequest{
		Logger: logger,
		Test:   true,
	}}
	r := gin.Default()
	...
	paymentAuthUrl := r.Group(fmt.Sprintf("%v/payment", "v2"), middleware.Authorize(db, trans.ExtReq, middleware.AuthType))
	{
		paymentAuthUrl.POST("/create", trans.CreatePayment)
	}
