...
func TestListDisbursementByAccountID(t *testing.T) {
	logger := tst.Setup()
	...
	{
		paymentApiUrl.GET("/disbursement/user/:account_id", paymnt.ListDisbursementByAccountID)
	}
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			URI := url.URL{Path: "/v2/disbursement/user/" + strconv.Itoa(test.AccountID)}
...
func TestWalletTransfer(t *testing.T) {
	...
	tests := []struct {
		Name         string
		RequestBody  models.WalletTransferRequest
		ExpectedCode int
		Headers      map[string]string
		Message      string
	}{
		{
			Name: "OK wallet transfer",
			RequestBody: models.WalletTransferRequest{
				SenderAccountID:    int(testUser.AccountID),
				RecipientAccountID: int(testUser.AccountID),
				FinalAmount:        200,
				SenderCurrency:     "NGN",
	...
	paymentApiUrl := r.Group(fmt.Sprintf("%v", "v2"), middleware.Authorize(db, paymnt.ExtReq, middleware.ApiType))
	{
		paymentApiUrl.POST("/disbursement/wallet/wallet-transfer", paymnt.WalletTransfer)
	}
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			var b bytes.Buffer
			json.NewEncoder(&b).Encode(test.RequestBody)
			URI := url.URL{Path: "/v2/disbursement/wallet/wallet-transfer"}
...
func TestManualDebit(t *testing.T) {
	logger := tst.Setup()
	gin.SetMode(gin.TestMode)
	validatorRef := validator.New()
	db := postgresql.Connection()
	...
	tests := []struct {
		Name         string
		RequestBody  models.ManualDebitRequest
		ExpectedCode int
		Headers      map[string]string
		Message      string
	}{
		{
			Name: "OK manual debit",
			RequestBody: models.ManualDebitRequest{
				AccountID:         int(testUser.AccountID),
				Amount:            200,
				Narration:         "narration",
			...
			ExpectedCode: http.StatusOK,
			Message:      "Wallet Disbursement Queued",
			Headers: map[string]string{
				"Content-Type":  "application/json",
				"v-private-key": pvKey,
				"v-public-key":  pbKey,
	...
	paymentApiUrl := r.Group(fmt.Sprintf("%v", "v2"), middleware.Authorize(db, paymnt.ExtReq, middleware.ApiType))
	{
		paymentApiUrl.POST("/disbursement/wallet/withdraw", paymnt.ManualDebit)
	}
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			var b bytes.Buffer
			json.NewEncoder(&b).Encode(test.RequestBody)
			URI := url.URL{Path: "/v2/disbursement/wallet/withdraw"}
...
func TestManualRefund(t *testing.T) {
	...
	tests := []struct {
		Name         string
		RequestBody  models.ManualRefundRequest
		ExpectedCode int
		Headers      map[string]string
		Message      string
	}{
	...
	paymentApiUrl := r.Group(fmt.Sprintf("%v", "v2"), middleware.Authorize(db, paymnt.ExtReq, middleware.ApiType))
	{
		paymentApiUrl.POST("/disbursement/process/refund", paymnt.ManualRefund)
	}
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			var b bytes.Buffer
			json.NewEncoder(&b).Encode(test.RequestBody)
			URI := url.URL{Path: "/v2/disbursement/process/refund"}
