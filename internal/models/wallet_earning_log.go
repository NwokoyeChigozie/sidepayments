...
func (w *WalletEarningLog) CreateWalletEarningLog(db *gorm.DB) error {
	err := postgresql.CreateOneRecord(db, &w)
	if err != nil {
		return fmt.Errorf("wallet earning log creation failed: %v", err.Error())
	}
	return nil
}
