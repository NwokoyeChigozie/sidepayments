	...
	if gatewayStatus {
		...
		if payment.TransactionID != "" {
			...
			if err == nil {
				...
				//credit vesicash
				err = CreditWallet(extReq, db, amountTwo, transaction.Currency, 1, false, "no", transaction.TransactionID)
				if err != nil {
					return "error", http.StatusInternalServerError, err
				}
				amountOne = totalAmount - amountOne
				err = CreditWallet(extReq, db, amountOne, transaction.Currency, buyerParty.AccountID, false, escrowWallet, transaction.TransactionID)
				if err != nil {
					return "error", http.StatusInternalServerError, err
				}

			} else {
				err = CreditWallet(extReq, db, payment.TotalAmount, transaction.Currency, buyerParty.AccountID, false, escrowWallet, transaction.TransactionID)
				if err != nil {
					return "error", http.StatusInternalServerError, err
				}
			}
