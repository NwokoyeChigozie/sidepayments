...
// $business_profile_data = BusinessProfile::where('account_id', $tx->business_id)->first();

//                         if ($business_profile_data) {
//                             if ($business_profile_data->webhook_uri != NULL) {
//                                 $this->initWebHook($business_profile_data->webhook_uri, 'bank-transfer.success', [
//                                     'reference'  => $payment_account->payment_account_id,
//                                     'amount'     => $amount,
//                                     'status'     => 'success',
//                                 ], $business_profile_data->account_id);
//                             }
//                         }

//                         return $this->response('ok', 'Bank Transfer Verified', ['reference' => $request->reference, 'amount' => $amount, 'pdf_link' => $this->pdf_link], 200);
//                     } catch (\Exception $e) {
//                         Log::error($e->getMessage());
//                     }
//                 } else {
//                     return $this->response('ok', 'Bank Transfer Not Verified', ['reference' => $request->reference, 'amount' => $amount, 'status' => $verify], 200);
//                 }
